IF OBJECT_ID('tempdb..#spBlockerTool') IS NOT NULL
	DROP PROCEDURE #spBlockerTool
GO
-----------------------------------------------------------------------------------------------------
--
--
-----------------------------------------------------------------------------------------------------
CREATE PROCEDURE #spBlockerTool(@timespan INT = 24)
AS
...
PRINT 'BLOCKER_Tool_SCRIPT_KATMAI Script v10.0.13 (SQL2008)'
PRINT ''
...
SET @time = GETDATE()
EXEC #spBlockerTool_0

WHILE @startDate < @endDate
BEGIN
	PRINT ''
	PRINT 'BLOCKER_Tool_BEGIN SqlMonData ' + CONVERT(VARCHAR(24), GETDATE(), 121)

	SET @startDate = GETDATE()
	
	EXEC #spBlockerTool_1 
	EXEC #spBlockerTool_1_handle
	
	IF @step % 12 = 0 
	BEGIN
		DECLARE @savePrevDate DATETIME = GETDATE()
		
		EXEC #spBlockerTool_2 @prevDate
		SET @prevDate = @savePrevDate
		
		EXEC #spBlockerTool_2_handle
	END

	SET @step = @step + 1
	
	PRINT ''
	PRINT 'BLOCKER_Tool_END SqlMonData ' + CONVERT(VARCHAR(24), GETDATE(), 121) + ' ' + convert(VARCHAR(12), datediff(ms,@startDate,getdate())) 
...
GO
-----------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#spBlockerTool_0') IS NOT NULL
	DROP PROCEDURE #spBlockerTool_0
	
IF OBJECT_ID('tempdb..#spBlockerTool_1') IS NOT NULL
	DROP PROCEDURE #spBlockerTool_1

IF OBJECT_ID('tempdb..#spBlockerTool_2') IS NOT NULL
	DROP PROCEDURE #spBlockerTool_2

IF OBJECT_ID('tempdb..#spBlockerTool_1_handle') IS NOT NULL
	DROP PROCEDURE #spBlockerTool_1_handle

IF OBJECT_ID('tempdb..#spBlockerTool_2_handle') IS NOT NULL
	DROP PROCEDURE #spBlockerTool_2_handle
-----------------------------------------------------------------------------------------------------
GO
CREATE PROCEDURE #spBlockerTool_0
AS
...
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN MachineInfo'
PRINT ''
...
PRINT ''
PRINT 'BLOCKER_Tool_END MachineInfo ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN @@version'
SELECT @@version AS 'version'
PRINT 'BLOCKER_Tool_END @@version ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

PRINT ''
PRINT 'BLOCKER_Tool_BEGIN xp_msver'
EXEC xp_msver
PRINT 'BLOCKER_Tool_END xp_msver ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_sys_info'
...
PRINT 'BLOCKER_Tool_END sys.dm_os_sys_info '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_cluster_nodes'
SELECT NodeName FROM sys.dm_os_cluster_nodes 
PRINT 'BLOCKER_Tool_END sys.dm_os_cluster_nodes '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_io_cluster_shared_drives'
SELECT DriveName FROM sys.dm_io_cluster_shared_drives 
PRINT 'BLOCKER_Tool_END sys.dm_io_cluster_shared_drives '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
 
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.configurations'
SELECT name, value=CAST(value AS VARCHAR(16)), value_in_use=CAST(value_in_use AS VARCHAR(16)) 
FROM sys.configurations
ORDER BY name

PRINT 'BLOCKER_Tool_END sys.configurations ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
	
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.databases'
SELECT 
...
PRINT 'BLOCKER_Tool_END sys.databases '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.master_files'
-- VIEW ANY DEFINITION
SELECT 
...
PRINT 'BLOCKER_Tool_END sys.master_files '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.master_files[Size]'
-- VIEW ANY DEFINITION
SELECT 
...
PRINT 'BLOCKER_Tool_END sys.master_files[Size] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.traces'
-- VIEW ANY DEFINITION
SELECT 
...
FROM sys.traces t
PRINT 'BLOCKER_Tool_END sys.traces '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
...
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_exec_sessions'
...
PRINT 'BLOCKER_Tool_END sys.dm_exec_sessions '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
	
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_exec_connections/sessions'
...
PRINT 'BLOCKER_Tool_END sys.dm_exec_connections/sessions '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
	
GO
-----------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------
CREATE PROCEDURE #spBlockerTool_1
AS
...
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_exec_requests'
...
PRINT 'BLOCKER_Tool_END sys.dm_exec_requests '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_exec_cursors'
...
PRINT 'BLOCKER_Tool_END sys.dm_exec_cursors '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_exec_query_memory_grants'
...
PRINT 'BLOCKER_Tool_END sys.dm_exec_query_memory_grants '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_tasks'
SELECT
...
PRINT 'BLOCKER_Tool_END sys.dm_os_tasks '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_waiting_tasks'
SELECT
...
WHERE session_id IN (SELECT session_id FROM sys.dm_exec_sessions WHERE is_user_process=1)
PRINT 'BLOCKER_Tool_END sys.dm_os_waiting_tasks  '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 


SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_tran_database_transactions'
...
PRINT 'BLOCKER_Tool_END sys.dm_tran_database_transactions '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

GO

-----------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------
CREATE PROCEDURE #spBlockerTool_1_handle
AS
...
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN CollectSqlHandle'
...
PRINT 'BLOCKER_Tool_END CollectSqlHandle ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
GO

-----------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------
CREATE PROCEDURE #spBlockerTool_2(@prevDate DATETIME)
AS
...
SELECT @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_resource_governor_resource_pools'
...
PRINT 'BLOCKER_Tool_END sys.dm_resource_governor_resource_pools '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SELECT @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_resource_governor_workload_groups'
...
PRINT 'BLOCKER_Tool_END sys.dm_resource_governor_workload_groups '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_schedulers'
...
PRINT 'BLOCKER_Tool_END sys.dm_os_schedulers '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 


SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_waiting_tasks[BlockedSessions]';
...
PRINT 'BLOCKER_Tool_END sys.dm_os_waiting_tasks[BlockedSessions] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 


SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_sys_memory'
-- SQL 2008
SELECT 
...
PRINT 'BLOCKER_Tool_END sys.dm_os_sys_memory '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_process_memory'
-- SQL 2008
SELECT
...
PRINT 'BLOCKER_Tool_END dm_os_process_memory '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
 
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_memory_clerks'
...
PRINT 'BLOCKER_Tool_END sys.dm_os_memory_clerks '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_memory_brokers'
...
PRINT 'BLOCKER_Tool_END sys.dm_os_memory_brokers '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
  
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_tran_active_transactions'
...
PRINT 'BLOCKER_Tool_END sys.dm_tran_active_transactions '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
 
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_tran_session_transactions'

SELECT
session_id, transaction_id, is_user_transaction AS 'is_user', is_local
FROM sys.dm_tran_session_transactions

PRINT 'BLOCKER_Tool_END sys.dm_tran_session_transactions '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_tran_active_snapshot_database_transactions'
...
PRINT 'BLOCKER_Tool_END sys.dm_tran_active_snapshot_database_transactions '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_exec_requests[System]'
...
PRINT 'BLOCKER_Tool_END sys.dm_exec_requests[System] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 


SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN dm_os_tasks[System]'
SELECT
...
PRINT 'BLOCKER_Tool_END sys.dm_os_tasks[System] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 


SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_exec_sessions[last_request_time]'
...
PRINT 'BLOCKER_Tool_END sys.dm_exec_sessions[last_request_time] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
...
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_wait_stats'
...
PRINT 'BLOCKER_Tool_END sys.dm_os_wait_stats '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
 
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_latch_stats'
...
PRINT 'BLOCKER_Tool_END sys.dm_os_latch_stats '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 


SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_os_spinlock_stats'
...
PRINT 'BLOCKER_Tool_END sys.dm_os_spinlock_stats '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_exec_query_resource_semaphores'
...
PRINT 'BLOCKER_Tool_END sys.dm_exec_query_resource_semaphores '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
 
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_io_pending_io_requests'
...
PRINT 'BLOCKER_Tool_END sys.dm_io_pending_io_requests '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

IF @pending_io_requests_summary = 1 
BEGIN
	SET @time = GETDATE()
	PRINT ''
	PRINT 'BLOCKER_Tool_BEGIN sys.dm_io_pending_io_requests[summary]'
	...
	PRINT 'BLOCKER_Tool_END sys.dm_io_pending_io_requests[summary] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
END
	 
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_io_virtual_file_stats'
...
PRINT 'BLOCKER_Tool_END sys.dm_io_virtual_file_stats '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.databases[log_reuse_wait_desc]'

SELECT d.name, d.log_reuse_wait_desc
FROM sys.databases d 
WHERE d.log_reuse_wait <> 0

PRINT 'BLOCKER_Tool_END sys.databases[log_reuse_wait_desc] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
 
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN DBCC_SQLPERF_LOGSPACE'

DBCC SQLPERF(LOGSPACE)

PRINT 'BLOCKER_Tool_END DBCC_SQLPERF_LOGSPACE '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SELECT @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN DBCC_OPENTRAN'
...
PRINT 'BLOCKER_Tool_END DBCC_OPENTRAN '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_tran_locks[OBJECTS]'
SELECT
...
PRINT 'BLOCKER_Tool_END sys.dm_tran_locks[OBJECTS] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 


SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN sys.dm_tran_locks[WAIT]'
SELECT
...
PRINT 'BLOCKER_Tool_END sys.dm_tran_locks[WAIT] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

GO

-----------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------
CREATE PROCEDURE #spBlockerTool_2_handle
AS
...
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN CollectSqlHandle2'
...
PRINT 'BLOCKER_Tool_END CollectSqlHandle2 ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 


SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN FlushSqlHandle[object_name]'
...
PRINT 'BLOCKER_Tool_END FlushSqlHandle[object_name] ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN FlushSqlHandle[sqlquery_requested]'
SELECT
'COUNT=',			Count=COUNT(*),
...
PRINT 'BLOCKER_Tool_END FlushSqlHandle[sqlquery_requested] ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 

SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_Tool_BEGIN FlushSqlHandle[dm_exec_sql_text]'
SELECT TOP 1000
'COUNT=',			COUNT(*),
'SQLHANDLE=',		CAST(req.sql_handle AS VARBINARY(26)) AS 'sql_handle',
...
PRINT 'BLOCKER_Tool_END FlushSqlHandle[dm_exec_sql_text] ' + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
GO
