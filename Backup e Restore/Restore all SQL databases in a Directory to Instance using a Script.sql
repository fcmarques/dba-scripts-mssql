...
--			Any database backup that matches the form %_FULL_% will be restored to
...
--			file name.  For  example Insurance_FULL_200305212302.BAK will be restored to
--			a database named Insurance.  The characters preceeding the '_FULL_' text determines
--			the name.
--
-- Input Parameters: 
--          @restoreFromDir - The directory where the database backups are located
--			@restoreToDataDir - The directory where the data files (i.e. MDF) will be restored to
--			@restoreToLogDir - The directory where the log files (i.e. LDF) will be restored to.  If this parameter is not provided then the log files are restored to @restoreToDataDir.
--          @MatchFileList - set to 'Y' to restore to same directory structure contained in the backup, also allows for secondary data files 'ndf' to to be in a different dir than mdf files
...
select * from #dirList where filename like '%_FULL_%' --order by filename

if @OneDBName is null 
   declare BakFile_csr cursor for 
  	select * from #dirList where filename like '%_FULL_%bak' order by filename
else
   begin  -- single db, don't order by filename, take default latest date /o-d parm in dir command above
     select @searchName = @OneDBName + '_FULL_%bak'
     declare BakFile_csr cursor for 
...
while @@fetch_status = 0
   begin
       ...
       if @OneDBName is null 
          select @dbName = left(@filename,datalength(@filename) - patindex('%_LLUF_%',reverse(@filename))-5)
       else
