...
/***********************************************************************************************************************
    ...
    SQL 2005 or greater
        ...
        Version 1.42
            ...
            Uses IsGUID function to not defrag DB's with GUID's in name (errors in object name appear if a GUID is in the DB name)
                ...
                **Basically, if a DB name ends in a GUID (like many sharepoint related DB's), there is something in my
                    ...
                    the database degragged nicely with the hyphens and the underscores. This problem seems solely related
                    to GUIDs.
            10 Oct 2011
        ...
        Version 1.44
            Check for bracked DB Names in input parameters, SSMS 2011 adds brackets if names are dragged/dropped
            07 Dec 2011
...
create procedure [dbo].[sp_Defrag_Indexes]
    ...
    --Check that SSMS did not bracket DB Names, common in SSMS 2012
...
/*********************************************************************************************************************
    ...
    NB: these check only apply if the database name is specified; it does not apply to all database.
        When all databases are selected into the cursor, it only selects thos which meet this criteria
...
*********************************************************************************************************************/
    ...
    print ' Excluding Version 80/2000 or Earlier Databases: ' + @Listof2000DBs
    ...
    print ' SQL Edition: ' + @SQLEdition + ' Edition ' + @WhiteSpace
    ...
    print @WhiteSpace
    print @WhiteSpace
    print '                                        Begin Operations on DB(s)' + @WhiteSpace
    print @WhiteSpace
    print @WhiteSpace
...
ExecuteForEachDatabase:
                ...
                *****************************************************************************************/
                if len(@DBName) >= 38
                    begin
                        ...
                        if ((dbo.IsGUID(@DBNameGUID) = 1) and (@DBName = 'all'))
                            begin
                            ...
                            else if ((dbo.IsGUID(@DBNameGUID) = 1) and (@DBName <> 'all'))
                                begin
                                    set @HandledError = @HandledError + 1

                                    print '' + @WhiteSpace
                                    print 'ERROR: ' + @WhiteSpace
                                    print '        The Database Name contains an invalid set of characters (GUID at end of DB Name).'
                                    print '        Aborting defragmentation process...'
                                    print '' + @WhiteSpace
                                    goto Finish
                                end
                    end
                ...
                *********************************************************************************************************************/
                ...
                if @RO = 'read_only'
                        begin
                            set @ErrorTxt = 'Database ' + @DB + ' is read only. No operations can be performed.' + @WhiteSpace
                            print 'Continuing on to next database...' + @WhiteSpace
                            print @ErrorTxt + @WhiteSpace
                            goto NextDB
                        end
                ...
                if @SingleUser = 'single_user'
                        begin
                            set @ErrorTxt = 'Database ' + @DB + ' is in Single User only mode. No operations can be performed.' + @WhiteSpace
                            print 'Continuing on to next database...' + @WhiteSpace
                            print @ErrorTxt + @WhiteSpace
                            goto NextDB
                        end
                        ...
                        begin
                            ...
                            *********************************************************************************************************************/
                            ...
                            set @FragTxt = ' -->> Fragmentation had been: ' + cast(cast((cast((@FragOld * 100) as int)) as real) / 100 as varchar)
                                 + '% <<->> Index Page Count = ' + cast(@PageCount as varchar) + ' <<<----->>> @ ' + cast(getdate() as varchar) + ' '
                            ...
                            if ((len(@IndexExists) > 2) or (len(ltrim(rtrim(@DBName))) = 0))
                                begin
                                    ...
                                    if @ExecAlter = 'true'
                                        ...
                                        begin catch
                                            ...
                                            if error_number() = 2725
                                                begin
                                                    set @HandledError = @HandledError + 1
                                                    print '--*****ERROR: cannot build online index for data type text, ntext, image, varchar(max), nvarchar(max), varbinary(max), xml, or large CLR type.'
                                                    print '--*****Index Name: ' + quotename(@DBName) + '.' + @SchemaName + '.' + @ObjectName
                                                    print '--*****Setting to offline rebuild for index ' + quotename(@DBName) + '.' + @SchemaName + '.' + @ObjectName
                                                    set @SQLCmdWith = ' with ('
                                                    ...
                                                    print (@SQLCmd) + ' --*****ERROR RESOLVED (err: 2725)***** --> OFFLINE Operation Required'
                                                    exec (@SQLCmd)
                                                end --error 2725
                                            ...
                                            if error_number() = 1934
                                                begin
                                                    ...
                                                    print (@SQLCmd) + ' --*****ERROR RESOLVED (err: 1934)***** --> set set concat_null_yields_null on'
                                                    exec (@SQLCmd)
                                                    set concat_null_yields_null off
                                                end --error 1934
                                            ...
                                            if error_number() = 1943
                                                begin
                                                    set @HandledError = @HandledError + 1
                                                    print '    --*****Handled Error Number 1943'
                                                    print ' --*****The index cannot be reorganized or rebuilt because page level locking is disabled.'
                                                    print ' --*****Index Name: ' + quotename(@DBName) + '.' + @SchemaName + '.' + @ObjectName
                                                    print ' --*****Error Code: ' + @SQLCmd
                                                    print @WhiteSpace
                                            ...
                                            --Unhandled Errors
                                            if (error_number() not in (2725, 1934, 1943))
                                                begin
                                                    ...
                                                    print '        --*****Object Name: ' + quotename(@DBName) + '.' + @SchemaName + '.' + @ObjectName
                                                    print '        --*****Error Code: ' + @SQLCmd
                                                    print @WhiteSpace
                                    ...
                                    begin
                                        ...
                                        print 'Index Name Error: ' + quotename(@DBName) + '.' + @SchemaName + '.' + @ObjectName
