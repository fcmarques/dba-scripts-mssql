...
DECLARE @deadlock TABLE (
        DeadlockID BIGINT IDENTITY PRIMARY KEY CLUSTERED,
        DeadlockGraph XML
        );
...
FROM    (SELECT CAST(target_data AS XML) AS TargetData
         ...
         WHERE  name = 'system_health' AND st.target_name = 'ring_buffer'
        ) AS Data
...
SELECT  CTE.DeadlockID,
        ...
        MainLock.Process.value('@dbid', 'BIGINT') AS Database_id,
