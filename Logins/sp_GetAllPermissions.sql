...
-- Convert into System Object
EXEC sp_ms_marksystemobject 'sp_GetAllPermissions' 
GO 

-- filter result

exec dbo.sp_GetAllPermissions

SELECT *
FROM
    OPENROWSET('SQLNCLI', 'Server=(local);database=sicc;TRUSTED_CONNECTION=YES;', 'exec sp_GetAllPermissions')
WHERE ObjectName = 'm_ajuste_lancado'
UNION
SELECT *
FROM
    OPENROWSET('SQLNCLI', 'Server=(local);database=sicc_band;TRUSTED_CONNECTION=YES;', 'exec sp_GetAllPermissions')
WHERE ObjectName = 'm_ajuste_lancado'
;