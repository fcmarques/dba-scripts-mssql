...
DECLARE @T_Obj TABLE (DatabaseName SYSNAME, UserName SYSNAME, ObjectName SYSNAME, Permission NVARCHAR(128))
SET @Obj='
USE ?
SELECT ''?'' AS DatabaseName, Us.name AS username, Obj.name AS object,  dp.permission_name AS permission 
FROM sys.database_permissions dp
JOIN sys.sysusers Us 
ON dp.grantee_principal_id = Us.uid 
JOIN sys.sysobjects Obj
ON dp.major_id = Obj.id 
where Obj.name = ''m_ajuste_lancado'''
INSERT @T_Obj 
