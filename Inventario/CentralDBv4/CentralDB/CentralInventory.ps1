...
param(
	...
	[string]$LogPath="c:\CentralDB\Errorlog\CentralInventorylog.log"
	)
...
CREATE TABLE #tb_db_files_space(
	...
	[SizeInMB] [int] NULL,
	[FreeSpaceInMB] [decimal](10, 2) NULL,
	[UsedSpaceInMB] [decimal](10, 2) NULL,
	[PctFreeSpace] [decimal](5, 2) NULL,
	[GrowthPct] [int] NULL,
	[GrowthInMB] [int] NULL,
...
SELECT
	...
	CAST((f.size/128.0) AS DECIMAL(15,2)) AS [SizeInMB],
	CAST(f.size/128.0 - CAST(FILEPROPERTY(f.name, ''SpaceUsed'') AS int)/128.0 AS DECIMAL(15,2)) AS [FreeSpaceInMB], 
	FILEPROPERTY(f.name, ''SpaceUsed'')/128 AS [UsedSpaceInMB],
	CAST(((CAST(f.size/128.0 - CAST(FILEPROPERTY(f.name, ''SpaceUsed'') AS int)/128.0 AS DECIMAL(15,2))) / (CAST((f.size/128.0) AS DECIMAL(15,2)))) * 100 AS DECIMAL(15,2)) AS [PctFreeSpace],
	...
	CASE ( is_percent_growth ) 
         ...
         ELSE 0 
       END                  AS GrowthInMB,
	NULLIF (fg.name, ''LOG'') AS [FileGroupName]
FROM sys.database_files AS f WITH (NOLOCK) 
LEFT OUTER JOIN sys.data_spaces AS fg WITH (NOLOCK) ON f.data_space_id = fg.data_space_id 
order by 1,2
OPTION (RECOMPILE);
'
select ('$Svr') as ServerName, ('$inst') as InstanceName,*,('$RunDt') as DateAdded from #tb_db_files_space
...
{ 
        $ex = $_.Exception 
        write-log -Message "$ex.Message on $svr While Collecting DBGrwoth Info" -Level Error -NoConsoleOut -Path $LogPath
}finally{
   $ErrorActionPreference = "Continue"; #Reset the error action pref to default
}
...
{ 
        $ex = $_.Exception 
        write-log -Message "$ex.Message on $svr While Collecting Hekaton Tables Info" -Level Error -NoConsoleOut -Path $LogPath
}finally{
   $ErrorActionPreference = "Continue"; #Reset the error action pref to default
}
...
{ 
        $ex = $_.Exception 
        write-log -Message "$ex.Message on $svr while collecting Server and SQL Info" -Level Error -NoConsoleOut -Path $LogPath
}  
