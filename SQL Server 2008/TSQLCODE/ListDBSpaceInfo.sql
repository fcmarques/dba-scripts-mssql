...
SELECT t.[DB Name] AS [Database Name]
      , t.[Schema Name]
      , t.[Table Name]
      , dbo.Commas(t.[Row Count]) AS [Row Count]
      , dbo.Commas(t.[Reserved MB]) AS [Reserved MB]
      , dbo.Commas(t.[Data MB]) AS [Data MB]
      , dbo.Commas(t.[Index MB]) AS [Index MB]
...
BEGIN
    ...
    SELECT DB_NAME() AS [Database Name] 
        , dbo.Commas(@DataCurrentSize) AS [Current DB Size MB]
        , CASE WHEN @DataMaxSize < 1 THEN ''Unlimited'' ELSE dbo.Commas(@DataMaxSize) END AS [Max DB Size MB]
        , CASE WHEN @DataMaxSize < 1 THEN ''Unlimited'' ELSE dbo.Commas(@DataMaxSize - @UsedSpace) END AS [Potential DB Free Space MB]
        , dbo.Commas(@LogSize) AS [Tran Log Size MB]
        , dbo.Commas(@LogSpaceUsed) AS [% Log Used]
        , CASE WHEN @LogMaxSize < 1 THEN ''Unlimited'' ELSE dbo.Commas(@LogMaxSize) END AS [Max Log Size MB]
        , CASE WHEN @LogMaxSize < 1 THEN ''Unlimited'' ELSE dbo.Commas(@LogMaxSize - @LogSpaceUsed) END AS [Potential Log Free Space MB ]
     ;
END
'
