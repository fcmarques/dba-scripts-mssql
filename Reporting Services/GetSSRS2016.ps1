...
function main{ 
    ...
    foreach ($result in $ssrsinstance){ 
        ...
        $ssrsURL = Get-WmiObject -ComputerName $computer -class MSReportServer_ConfigurationSetting -namespace "root\Microsoft\SqlServer\ReportServer\$($result.name)\v13\admin" 
    ...
    # Starts Web Service Call 
        ...
        #Gets SSRS Version 
        $SQL = Get-WmiObject -computer $computer -class MSReportServer_Instance -namespace "root\Microsoft\SqlServer\ReportServer\$($result.name)\v13" 
