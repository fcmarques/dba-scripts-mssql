...
-- Lists the top statements by average input/output usage for the current database  (Query 33) (Top IO Statements)
...
-- Possible Bad Indexes (writes > reads)  (Query 34) (Bad NC Indexes)
...
-- Missing Indexes for current database by Index Advantage  (Query 35) (Missing Indexes)
SELECT DISTINCT CONVERT(decimal(18,2), user_seeks * avg_total_user_cost * (avg_user_impact * 0.01)) AS [index_advantage], 
migs.last_user_seek, mid.[statement] AS [Database.Schema.Table],
mid.equality_columns, mid.inequality_columns, mid.included_columns,
migs.unique_compiles, migs.user_seeks, migs.avg_total_user_cost, migs.avg_user_impact,
...
-- Find missing index warnings for cached plans in the current database  (Query 36) (Missing Index Warnings)
...
-- Breaks down buffers used by current database by object (table, index) in the buffer cache  (Query 37) (Buffer Usage)
-- Note: This query could take some time on a busy instance
...
-- Get SQL Server Agent jobs and Category information (Query 45) (SQL Server Agent Jobs)
...
-- Get SQL Server Agent Alert Information (Query 46) (SQL Server Agent Alerts)
