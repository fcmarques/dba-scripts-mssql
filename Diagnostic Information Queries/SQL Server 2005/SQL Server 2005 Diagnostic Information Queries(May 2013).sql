...
-- Hardware Information for SQL Server 2005  (Query 4) (Hardware Info)
...
-- if you have not recycled your error log recently  (Query 5) (System Manufacturer)
...
-- Get processor description from Windows Registry  (Query 6) (Processor Description)
...
-- Get configuration values for instance  (Query 7) (Configuration Values)
...
-- File Names and Paths for TempDB and all user databases in instance (Query 8) (Database Filenames and Paths)
...
-- Recovery model, log reuse wait description, log file size, log usage size (Query 9) (Database Properties)
...
-- Missing Indexes for all databases by Index Advantage  (Query 10) (Missing Indexes All Databases)
...
-- Get VLF Counts for all databases on the instance (Query 11) (VLF Counts)
...
-- Calculates average stalls per read, per write, and per total input/output for each database file (Query 12) (IO Stalls by File)
...
-- Get CPU utilization by database (adapted from Robert Pearl) (Query 13) (CPU Usage by Database)
WITH DB_CPU_Stats
AS
...
-- Get I/O utilization by database (Query 14) (IO Usage By Database)
...
-- Get total buffer usage by database for current instance (Query 15) (Total Buffer Usage by Database)
...
-- Signal Waits for instance (Query 17) (Signal Waits)
...
--  Get logins that are connected and how many sessions they have  (Query 18) (Connection Counts)
...
-- Get Average Task Counts (run multiple times) (Query 19) (Avg Task Counts)
...
-- Get CPU Utilization History for last 256 minutes (in one minute intervals) (Query 20) (CPU Utilization History)
...
-- Page Life Expectancy (PLE) value for each NUMA node in current instance  (Query 21) (PLE by NUMA Node)
...
-- Memory Grants Pending value for current instance (Query 21) (Memory Grants Pending)
...
-- Memory Clerk Usage for instance (Query 22) (Memory Clerk Usage)
...
-- Find single-use, ad-hoc queries that are bloating the plan cache (Query 23) (Ad hoc Queries)
...
-- Individual File Sizes and space available for current database  (Query 24) (File Sizes and Space)
...
-- I/O Statistics by file for the current database  (Query 25) (IO Stats By File)
...
-- Cached SP's By Execution Count (SQL 2005)  (Query 26) (SP Execution Counts)
...
-- Top Cached SPs By Avg Elapsed Time (SQL 2005)  (Query 27) (SP Avg Elapsed Time)
...
-- Cached SP's By Worker Time (SQL 2005) Worker time relates to CPU cost  (Query 28) (SP Worker Time)
...
-- Cached SP's By Logical Reads (SQL 2005) Logical reads relate to memory pressure  (Query 29) (SP Logical Reads)
...
-- Top Cached SPs By Total Logical Writes (SQL 2005)  (Query 31) (SP Logical Writes)
...
-- Lists the top statements by average input/output usage for the current database  (Query 32) (Top IO Statements)
...
-- Possible Bad Indexes (writes > reads)  (Query 33) (Bad NC Indexes)
...
-- Missing Indexes for current database by Index Advantage  (Query 34) (Missing Indexes)
...
-- Find missing index warnings for cached plans in the current database  (Query 35) (Missing Index Warnings)
...
-- Breaks down buffers used by current database by object (table, index) in the buffer cache  (Query 36) (Buffer Usage)
-- Note: This query could take some time on a busy instance
...
-- Get Table names, row counts  (Query 37) (Table Sizes)
...
-- When were Statistics last updated on all indexes?  (Query 39) (Statistics Update)
...
-- Get fragmentation info for all indexes above a certain size in the current database  (Query 40) (Index Fragmentation)
...
--- Index Read/Write stats (all tables in current DB) ordered by Reads  (Query 41) (Overall Index Usage - Reads)
...
--- Index Read/Write stats (all tables in current DB) ordered by Writes  (Query 42) (Overall Index Usage - Writes)
SELECT OBJECT_NAME(s.[object_id]) AS [ObjectName], i.name AS [IndexName], i.index_id,
...
-- Look at recent Full backups for the current database (Query 43) (Recent Full Backups)
...
-- Get the average full backup size by month for the current database (Query 44) (Database Size History)
