...
-- Calculates average stalls per read, per write, and per total input/output for each database file  (Query 17) 
...
-- Get total buffer usage by database for current instance  (Query 19)
...
-- Isolate top waits for server instance since last restart or statistics clear  (Query 20)
WITH Waits AS
...
-- Signal Waits for instance  (Query 21)
...
--  Get logins that are connected and how many sessions they have  (Query 22) 
...
-- Get Average Task Counts (run multiple times)  (Query 23)
...
-- Get CPU Utilization History for last 256 minutes (in one minute intervals)  (Query 24)
...
-- Good basic information about OS memory amounts and state  (Query 25)
...
-- SQL Server Process Address space info  (Query 26) 
...
-- Page Life Expectancy (PLE) value for current instance  (Query 27)
...
-- Memory Grants Outstanding value for current instance  (Query 28)
...
-- Memory Grants Pending value for current instance  (Query 29)
...
-- Memory Clerk Usage for instance  (Query 30)
...
-- Find single-use, ad-hoc queries that are bloating the plan cache  (Query 31)
...
-- Individual File Sizes and space available for current database  (Query 32)
...
-- I/O Statistics by file for the current database  (Query 33)
...
-- Get VLF count for transaction log for the current database,  (Query 34)
...
-- Top cached queries by Execution Count (SQL Server 2008)  (Query 35)
...
-- Top Cached SPs By Execution Count (SQL 2008) (SQL 2008 R2 SP1 only) (Query 36)
...
-- Top Cached SPs By Avg Elapsed Time (SQL 2008)  (Query 37)
...
-- Top Cached SPs By Avg Elapsed Time with execution time variability   (Query 38)
...
-- Top Cached SPs By Total Worker time (SQL 2008). Worker time relates to CPU cost  (Query 39)
...
-- Top Cached SPs By Total Logical Reads (SQL 2008). Logical reads relate to memory pressure  (Query 40)
...
-- Top Cached SPs By Total Physical Reads (SQL 2008). Physical reads relate to disk I/O pressure  (Query 41)
...
-- Lists the top statements by average input/output usage for the current database  (Query 43)
...
-- Possible Bad NC Indexes (writes > reads)  (Query 44)
...
-- Missing Indexes for current database by Index Advantage  (Query 45)
...
-- Find missing index warnings for cached plans in the current database  (Query 46)
...
-- Breaks down buffers used by current database by object (table, index) in the buffer cache  (Query 47)
-- Note: This query could take some time on a busy instance
...
-- Get Table names, row counts, and compression status for clustered index or heap  (Query 48)
...
-- When were Statistics last updated on all indexes?  (Query 49)
...
-- Get fragmentation info for all indexes above a certain size in the current database  (Query 50) 
...
--- Index Read/Write stats (all tables in current DB) ordered by Reads  (Query 51)
...
--- Index Read/Write stats (all tables in current DB) ordered by Writes  (Query 52)
SELECT OBJECT_NAME(s.[object_id]) AS [ObjectName], i.name AS [IndexName], i.index_id,
...
-- Look at recent Full backups for the current database (Query 53)
