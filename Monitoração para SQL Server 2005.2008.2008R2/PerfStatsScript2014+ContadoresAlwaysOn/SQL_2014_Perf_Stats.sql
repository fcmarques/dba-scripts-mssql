...
GO
IF (CHARINDEX ('12.0', @@VERSION) = 0) BEGIN
  PRINT ''
  PRINT '**** NOTE ****'
  PRINT '**** This script is for SQL Server 2014.  Errors are expected when run on earlier versions.'
  PRINT '**************'
  PRINT ''
END
GO
PRINT 'Starting SQL Server 2014 Perf Stats Script...'
SET LANGUAGE us_english
PRINT '-- Script Source --'
SELECT 'SQL 2014 Perf Stats Script' AS script_name, '$Revision: 12 $ ($Change: 3355 $)' AS revision
PRINT ''
...
PRINT 'Script Name              SQL 2014 Perf Stats Script'
PRINT 'Script File Name         $File: SQL_2014_Perf_Stats.sql $'
...
--  EXEC sp_executesql @sql, N'@runtime datetime', @runtime = @runtime
  ...
  ** To conserve space, output full dbname and filenames on 1st execution only. */
  PRINT ''
  RAISERROR ('-- File Stats (new) --', 0, 1) WITH NOWAIT;
   SET @querystarttime = GETDATE()
  select @runtime 'runtime', db_name(database_id) 'Database_Name',  * from sys.dm_io_virtual_file_stats (null, null)
  ...
  ** dm_exec_query_memory_grants. */
  ...
  select @runtime as runtime, * from sys.dm_os_schedulers
