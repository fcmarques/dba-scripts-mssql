    ...
    { 
        if($item.IsSystemObject -eq $false) # Exclude system objects 
        {
            ...
            { 
				...
				$output += "`n<li><a href=`"$path" + ($item.Schema -replace "\\", "-") + "." + $item.Name + ".html`">" + $item.Schema + "." + $item.Name + "</a></li>"; 
            } 
        }
    ...
    { 
        ...
        { 
            ...
            { 
            	...
            	{ 
					$page = $filePath + $($db.Name + "\$objectName\" + ($item.Schema -replace "\\", "-") + "." + $item.Name + ".html"); 
                } 
                $title = ""; 
            	if([string]$item.GetType() -eq "Microsoft.SqlServer.Management.Smo.Schema") 
            		{ 
...
$filePath = $env:USERPROFILE + "\database_documentation\" + $Server + "\" + $Server + "_" + $dateTimeString + "\"; 
