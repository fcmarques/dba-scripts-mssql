...
*/
DECLARE @IncludeSystemTables bit,
    @MaxDop tinyint,
    @SortInTempDB bit,
    @DropExisting bit,
    @OnlineProcessing bit,
    @StatsRecompute bit

SELECT @IncludeSystemTables = 0,
    @MaxDop = (SELECT   CONVERT(tinyint, value_in_use)
              ...
              END + CASE WHEN I.type_desc = 'CLUSTERED' THEN 'CLUSTERED '
                    ...
                    END + CASE WHEN (I.is_unique_constraint = 0
                          ...
                          END + CASE WHEN (I.is_unique_constraint = 0
                                     ...
                                     ELSE ''
                                END + ' (' + IC.index_columns_key + ') ' + ISNULL('INCLUDE (' + IC.index_columns_include + ')', '') + 'WITH (' + CASE WHEN (i.allow_page_locks = 1) THEN 'ALLOW_PAGE_LOCKS = ON, '
