...
SELECT
    --RANK ( ) OVER (PARTITION BY v.Table_Name ORDER BY v.Column_Name)  AS RANK,
   ...
   ,'CREATE INDEX [ixnifk_' + v.Table_Name + '_' + FORMAT(RANK ( ) OVER (PARTITION BY v.Table_Name ORDER BY v.Column_Name), '00') + '] ON ' + QUOTENAME(DB_NAME()) + '.' + QUOTENAME(SCHEMA_NAME(fk.schema_id)) + '.' + QUOTENAME(v.Table_Name) + '([' + v.Column_Name + ']) WITH(ONLINE = ON);' AS	CreateScript
FROM 
