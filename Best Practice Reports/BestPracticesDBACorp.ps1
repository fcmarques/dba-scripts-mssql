...
ForEach-Object {
	...
	If ($createdpath)
	{
		...
		#Region Versão do banco de dados 
		
		Write-Log -Message "Checking the Versão do banco de dados...." -path $PathWriteLog -ErrorLogFileName $ErrorLogfileName -verbose
		Try
		{
			Invoke-Sqlcmd2 -ServerInstance $FullInstanceName -Database master -Query "
			SELECT 
			case 
				when Serverproperty( 'productversion' ) >= '10.50.6000.0' then cast(Serverproperty( 'productversion' ) as varchar) + ';Green' 
				when Serverproperty( 'productversion' ) < '10.50.6000.0' then cast(Serverproperty( 'productversion' ) as varchar) + ';Yellow' 
				when Serverproperty( 'productversion' ) < '10.50.3000.0' then cast(Serverproperty( 'productversion' ) as varchar) + ';RED' 
			end
				AS Versão,
			   Serverproperty ( 'productlevel' )  AS [Service Pack],
			   Serverproperty ( 'edition' )       AS Edição
			" |
			Select @{ N = 'Versão'; E = { $_.Versão } },
				   @{ N = 'Service Pack'; E = { $_.'Service Pack' } },
				   @{ N = 'Edição'; E = { $_.Edição } } |
			Export-Csv -force -NoClobber -NoTypeInformation -Path "$($FileName)VersãoDoBancoDeDados.csv"
		...
		#EndRegion Versão do banco de dados 
